...
import logging
import sys
from src.tools.initialize_tools import get_registry, discover_and_initialize_tools, console_prompt_handler
...
async def main():
    ...
    logger.debug(f"Registry state before discovery: approved_tools={registry.approved_tools}, tools={registry.tools.keys() if registry.tools else 'none'}")
    ...
    logger.debug(f"Registry state after discovery: approved_tools={registry.approved_tools}, tools={registry.tools.keys() if registry.tools else 'none'}")

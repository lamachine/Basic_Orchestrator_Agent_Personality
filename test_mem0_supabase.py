...
def test_memory_with_supabase():
    ...
    if not os.path.exists(config_path):
        ...
        print(
            f"  Please make sure the mem0.config.json file exists with proper Supabase configuration."
        )
        sys.exit(1)
    ...
    try:
        ...
        if "vector_store" not in config or config["vector_store"].get("provider") != "supabase":
            ...
            print(
                f"  Current vector_store provider: {config.get('vector_store', {}).get('provider')}"
            )
    ...
    try:
        ...
        try:
            ...
            for idx, item in enumerate(results.get("results", [])):
                print(
                    f"  Result {idx+1}: {item.get('content', '')} (Score: {item.get('similarity', 0)})"
                )
        except Exception as e:

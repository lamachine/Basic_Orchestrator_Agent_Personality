...
async def advisor(mock_services):
    ...
    with (
        patch("src.services.llm_services.get_llm_service", return_value=llm),
...
class TestAdvisorAnalysis:
    ...
    async def test_analyze_requirements_success(self, advisor, mock_services):
        ...
        llm, _ = mock_services
        llm.generate.side_effect = ["Initial analysis", json.dumps(SAMPLE_SPEC)]
    ...
    async def test_analyze_requirements_handles_bad_json(self, advisor, mock_services):
        ...
        llm, _ = mock_services
        llm.generate.side_effect = ["Analysis", "Not valid JSON"]

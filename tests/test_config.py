...
def test_repr_method():
    ...
    config = Configuration(anon_key="secret-anon-key", service_role_key="secret-service-key")
...
"""
    ...
    assert (
        config.providers.openai is None
        or not hasattr(config.providers.openai, "enabled")
        or not config.providers.openai.enabled
    )
...
"""
    ...
    assert (
        config.providers.ollama is None
        or not hasattr(config.providers.ollama, "enabled")
        or not config.providers.ollama.enabled
    )
...
"""
    ...
    assert (
        not hasattr(config.providers, "anthropic") or getattr(config.providers, "anthropic") is None
    )
    os.remove(path)

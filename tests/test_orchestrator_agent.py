...
def mock_llm_service():
    ...
    llm_service.query_llm = AsyncMock(return_value={"response": "Test response", "tool_calls": []})
    return llm_service
...
def orchestrator_agent(mock_llm_service, mock_tool_registry):
    ...
    return OrchestratorAgent(llm_service=mock_llm_service, tool_registry=mock_tool_registry)
...
async def test_process_message_with_tool_call(
    orchestrator_agent, mock_message_state, mock_llm_service, mock_tool_registry
):
...
async def test_process_message_error_handling(
    orchestrator_agent, mock_message_state, mock_llm_service
):

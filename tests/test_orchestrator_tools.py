...
from src.tools.orchestrator_tools import add_tools_to_prompt, format_tool_results, handle_tool_calls
...
def test_handle_tool_calls(mock_execute, mock_extract):
    ...
    # Set up mocks
    mock_extract.return_value = [{"name": "valet", "parameters": {"task": "check schedule"}}]
...
def test_format_tool_results(mock_format):
    ...
    results = {"execution_results": [{"name": "valet", "result": {"message": "Success!"}}]}
    formatted = format_tool_results(results)

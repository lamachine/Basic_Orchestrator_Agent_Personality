...
def config():
    ...
    return PersonalAssistantConfig(gmail_enabled=True, slack_enabled=True)
...
async def test_agent_initialization(config, mock_gmail_tool, mock_slack_tool, monkeypatch):
    ...
    # Mock tool creation
    monkeypatch.setattr(
        "src.sub_graph_personal_assistant.tools.gmail.GmailTool",
        lambda _: mock_gmail_tool,
    )
    monkeypatch.setattr(
        "src.sub_graph_personal_assistant.tools.slack_tool.SlackTool",
        lambda _: mock_slack_tool,
    )
...
async def test_process_slack_task(config, mock_slack_tool, monkeypatch):
    ...
    result = await agent.process_task("send slack message to #general: Hello team")

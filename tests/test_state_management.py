...
class TestStateValidator:
    ...
    def test_message_sequence_valid(self, validator):
        ...
        messages = [
            Message(role=MessageRole.USER, content="Hello", created_at=datetime.now()),
            Message(
    ...
    def test_message_sequence_invalid(self, validator):
        ...
        messages = [
            ...
            Message(role=MessageRole.ASSISTANT, content="Hi", created_at=datetime.now()),
        ]
...
class TestStateManager:
    def test_update_conversation(self, state_manager):
        ...
        message = state_manager.update_conversation(
            role=MessageRole.USER, content="Hello", metadata={"source": "test"}
        )
    ...
    def test_rate_limiting(self, state_manager):
        ...
        for _ in range(100):  # Should work
            state_manager.update_conversation(role=MessageRole.USER, content="test", metadata={})

        with pytest.raises(StateUpdateError, match="Too many rapid state updates"):
            for _ in range(101):  # Should fail
                state_manager.update_conversation(
                    role=MessageRole.USER, content="test", metadata={}
                )
    ...
    def test_task_lifecycle(self, state_manager):
        ...
        assert (
            state_manager.state["conversation_state"].current_task_status == TaskStatus.IN_PROGRESS
        )
    ...
    def test_error_stats(self, state_manager):
        ...
        try:
            state_manager.update_conversation(
                role=MessageRole.USER, content="", metadata={}  # Should cause error
            )
...
class TestStateIntegration:
    def test_complete_workflow(self, state_manager):
        ...
        # Add messages
        state_manager.update_conversation(
            role=MessageRole.USER, content="Request", metadata={"type": "command"}
        )
        state_manager.update_conversation(
            role=MessageRole.ASSISTANT, content="Response", metadata={"type": "result"}
        )

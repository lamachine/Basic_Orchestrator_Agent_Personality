...
def mock_supabase():
    ...
    mock_rpc.execute.return_value = MagicMock(
        data=[{"session_id": 123, "role": "user", "message": "Hello!", "similarity": 0.9}]
    )
...
def test_init_db_from_env(mock_create_client):
    ...
    with patch.dict(
        ...
        mock_create_client.assert_called_once_with("https://env.supabase.co", "env-key")
...
def test_load_conversation(db_manager, mock_supabase):
    ...
    mock_metadata_select.execute.return_value = MagicMock(
        data=[
            {
                "user_id": "developer",
                "metadata": json.dumps(
    ...
    mock_messages_select.execute.return_value = MagicMock(
        data=[
            {
...
def test_add_message(db_manager, mock_supabase):
    ...
    request_id = db_manager.add_message(
        session_id=123, role="user", message="Hello, world!", user_id="developer"
    )
...
def test_get_embedding(mock_llm_service, db_manager):
    ...
    db_manager.add_message(session_id=1, role="user", message="Test message", user_id="test-user")

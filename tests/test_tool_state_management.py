...
async def test_tool_state_persistence(mock_tool_with_state):
    ...
    result = await mock_tool_with_state.execute(
        ...
        parameters={"action": "add_task", "title": "Test state persistence", "due": "today"}
    )
...
async def test_tool_state_restoration(mock_tool_with_state):
    ...
    await mock_tool_with_state.execute(
        ...
        parameters={"action": "add_task", "title": "Task before restart", "due": "today"}
    )
    ...
    result = await restarted_tool.execute(
        ...
        parameters={"action": "add_task", "title": "Task after restart", "due": "tomorrow"}
    )
...
async def test_state_with_conversation_tracking(mock_tool_with_state):
    ...
    await mock_tool_with_state.execute(
        ...
        parameters={"action": "add_task", "title": "First conversation task", "due": "today"}
    )
    
    await mock_tool_with_state.execute(
        task_id="test-convo-2",
        parameters={"action": "list_tasks"}
    )
...
async def test_state_handles_missing_state_file(temp_state_dir):
    ...
    tool = PersonalAssistantWithState = type("PersonalAssistantWithState", (BaseModel,), {
        "__annotations__": {
            "name": str,
            "description": str,
            "state_file": str,
    ...
    result = await tool.execute(
        ...
        parameters={"action": "add_task", "title": "New task with missing state", "due": "today"}
    )

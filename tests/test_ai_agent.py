...
def test_query_llm_success(mock_config, mock_requests_post, mock_logger):
    ...
    with patch.object(agent, "has_db", False):  # Ensure DB doesn't interfere with test
        ...
        mock_requests_post.assert_called_once_with(
            ...
            json={"model": agent.model, "prompt": prompt, "stream": False},
        )
...
def test_continue_conversation(
    mock_config, mock_db_manager, mock_logger, sample_conversation_state
):
...
def test_continue_conversation_not_found(mock_config, mock_db_manager, mock_logger):
    ...
    mock_logger.error.assert_called_once()
    assert (
        "Failed to continue conversation: session 123 not found"
...
def test_update_task_status(mock_config, mock_db_manager, mock_logger, sample_conversation_state):
    ...
    assert result is True
    mock_db_manager.update_task_status.assert_called_once_with(
        sample_conversation_state, TaskStatus.IN_PROGRESS
    )
...
def test_update_task_status_failure(
    mock_config, mock_db_manager, mock_logger, sample_conversation_state
):
...
def test_chat_with_conversation_state(
    mock_config, mock_db_manager, mock_logger, sample_conversation_state
):
    ...
    agent.process_response_with_tools = Mock(
        return_value={"tool_calls": [], "execution_results": []}
    )
    mock_db_manager.update_task_status.return_value = True
    ...
    mock_db_manager.update_task_status.assert_called_with(
        sample_conversation_state, TaskStatus.COMPLETED
    )
...
def test_main_function_with_conversation_selection(
    mock_config, monkeypatch, mock_logger, sample_conversation_state
):

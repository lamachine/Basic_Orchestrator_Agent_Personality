...
try:
    from src.sub_graphs.template_agent.src.common.managers.memory_manager import (
        Mem0Memory,
        SwarmMessage,
    )
    ...
    try:
        ...
        for i, mem in enumerate(memories):
            message = SwarmMessage(
                content=mem["content"], user_id=user_id, metadata=mem["metadata"]
            )
            try:
        ...
        try:
            context_result = rag_engine.retrieve_context(query=query, user_id=user_id, limit=5)
            ...
            for i, item in enumerate(context_items):
                print(
                    f"  Item {i+1}: {item.get('content', '')} (Relevance: {item.get('relevance', 0):.2f})"
                )
                if "metadata" in item:
                    md = item["metadata"]
                    print(
                        f"    Type: {md.get('context_type', 'unknown')}, Source: {md.get('source', 'unknown')}"
                    )
        except Exception as e:
        ...
        try:
            task_context = rag_engine.retrieve_context(
                query="report deadline", user_id=user_id, context_type="task", limit=2
            )
        ...
        try:
            conversations = rag_engine.get_conversation_history(user_id=user_id, limit=5)
        ...
        try:
            ...
            if success:
                ...
                verification = rag_engine.retrieve_context(
                    query="groceries", user_id=user_id, context_type="task", limit=1
                )

                if verification.get("context") and len(verification["context"]) > 0:
                    print(
                        f"✓ Verified context was stored: {verification['context'][0].get('content', '')}"
                    )
                else:

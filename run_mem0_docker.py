...
def build_and_run():
    ...
    try:
        ...
        print(
            "Test with: curl -X POST -H 'Content-Type: application/json' -d '{\"method\":\"health\"}' http://localhost:8009/health"
        )
...
def stop():
    ...
    try:
        subprocess.run(["docker-compose", "-f", "docker-compose.mem0.yml", "down"], check=True)

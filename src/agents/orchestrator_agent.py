...
from src.tools.orchestrator_tools import add_tools_to_prompt, handle_tool_calls, execute_tool, TOOL_DEFINITIONS, format_tool_results, format_completed_tools_prompt, PENDING_TOOL_REQUESTS
from src.services.message_service import log_and_persist_message
from src.state.state_models import MessageRole, MessageType, MessageState
...
class OrchestratorAgent(BaseAgent):
    ...
    def __init__(self, config: Configuration = None, personality_file: Optional[str] = None):
        ...
        logger.debug(f"OrchestratorAgent.__init__: personality_enabled={config.personality_enabled}")
        logger.debug(f"OrchestratorAgent.__init__: config.personality_file_path={config.personality_file_path}")
        ...
        if personality_file:
            try:
                ...
                logger.error(f"Failed to load personality file {personality_file}: {e}", exc_info=True)
                self.personality_agent = None
        else:
    ...
    async def process_message(self, message: str, session_state: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if session_state:
            ...
            if "conversation_state" in session_state:
                logger.debug(f"[process_message] Conversation state type: {type(session_state['conversation_state'])}")
                if not isinstance(session_state["conversation_state"], MessageState):
                    logger.error(f"[process_message] Invalid conversation_state type: {type(session_state['conversation_state'])}")
                    raise TypeError(f"conversation_state must be MessageState, got {type(session_state['conversation_state'])}")
        ...
        if session_state and "conversation_state" in session_state:
            ...
            try:
                ...
                logger.error(f"[process_message] Failed to log user message: {str(e)}", exc_info=True)
        ...
        if session_state and "conversation_state" in session_state:
            ...
            try:
                ...
                logger.error(f"[process_message] Failed to log system prompt: {str(e)}", exc_info=True)
                raise  # Re-raise to handle at a higher level
        ...
        if session_state and "conversation_state" in session_state:
            ...
            try:
                ...
                if self.personality_agent and hasattr(self.personality_agent, 'personality'):
                    ...
                    logger.debug(f"[process_message] Using character name from personality: {character_name}")
                ...
                logger.error(f"[process_message] Failed to log LLM response: {str(e)}", exc_info=True)
                raise  # Re-raise to handle at a higher level
        ...
        if matches:
            ...
            try:
                ...
                logger.error(f"[process_message] Invalid tool call JSON: {tool_call_json} | Error: {e}")
            ...
            if session_state and "conversation_state" in session_state:
                ...
                try:
                    ...
                    logger.error(f"[process_message] Failed to log tool call: {str(e)}", exc_info=True)
                    raise  # Re-raise to handle at a higher level
            ...
            if session_state and "conversation_state" in session_state:
                ...
                try:
                    ...
                    logger.error(f"[process_message] Failed to log pending message: {str(e)}", exc_info=True)
                    raise  # Re-raise to handle at a higher level
        ...
        if session_state and "conversation_state" in session_state:
            ...
            try:
                ...
                logger.error(f"[process_message] Failed to log final response: {str(e)}", exc_info=True)
                raise  # Re-raise to handle at a higher level
    ...
    async def _create_prompt(self, message: str) -> str:
        ...
        logger.debug(f"orchestrator_agent:_create_prompt: Orchestrator role added: {self.prompt_section}")
        ...
        if self.personality_agent:
            ...
            logger.debug(f"orchestrator_agent:_create_prompt: Personality core added: {personality_header}")
            ...
            if is_first_message:
                ...
                if knowledge_items:
                    ...
                    logger.debug(f"orchestrator_agent:_create_prompt: Added knowledge: {knowledge_text}")
        ...
        if self.conversation_history:
            ...
            prompt_with_tools += f"\n\n{history_text}"
            logger.debug(f"orchestrator_agent:_create_prompt: Conversation history added: {history_text}")
    ...
    async def handle_tool_completion(self, request_id: str, original_query: str) -> Dict[str, Any]:
        ...
        """
        logger.debug(f"[handle_tool_completion] Processing tool completion for request {request_id}")
        
        try:
            # Get the tool result from PENDING_TOOL_REQUESTS
            if request_id not in PENDING_TOOL_REQUESTS:
                logger.warning(f"[handle_tool_completion] No pending request found for {request_id}")
                return {
            ...
            logger.debug(f"[handle_tool_completion] Found data for request {request_id}, tool: {tool_name}")

...
from src.tools.initialize_tools import get_registry, initialize_tools
...
def find_personality_file(
    config: Configuration, personality_file: Optional[str] = None
) -> Optional[str]:
    """
    ...
    try:
        ...
        if personality_file:
            ...
            if not file_path.exists():
                logger.error(
                    f"Personality file not found: {file_path} (absolute: {file_path.absolute()})"
                )
                return None
            logger.debug(
                f"Using explicitly provided personality file: {file_path} (exists: {file_path.exists()})"
            )
        ...
        if config.personality_file_path:
            ...
            logger.error(
                f"Default personality file not found: {file_path} (absolute: {file_path.absolute()})"
            )
...
def initialize_agents(
    config: Configuration, personality_file: Optional[str] = None
) -> Tuple[OrchestratorAgent, LLMQueryAgent]:
    """
...
async def run_with_interface(
    ...
    try:
        ...
        log_config = (
            config.user_config.get_logging_config() if hasattr(config, "user_config") else {}
        )
...
def main() -> int:
    ...
    parser.add_argument("--personality", "-p", help="Path to a personality JSON file")

...
class MessageState(BaseModel):
    ...
    async def add_message(self, role: MessageRole, content: str, metadata: Optional[Dict[str, Any]] = None, sender: str = None, target: str = None) -> Message:
        ...
        if not sender or not target:
            logger.error(f"[MessageState.add_message] sender or target missing: sender={sender}, target={target}")
        ...
        try:
            ...
            if self.db_manager and hasattr(self.db_manager, 'message_manager'):
                ...
                logger.debug(f"[MessageState.add_message] db_manager type: {type(self.db_manager)}")
                logger.debug(f"[MessageState.add_message] message_manager type: {type(self.db_manager.message_manager)}")
                try:
                    ...
                    logger.error(f"[MessageState.add_message] Failed to persist message: {str(e)}", exc_info=True)
                    # Continue even if database persistence fails
            else:
                logger.warning(f"[MessageState.add_message] No db_manager or message_manager, message only in memory")
                if self.db_manager:
                    logger.warning(f"[MessageState.add_message] db_manager exists but no message_manager attribute")
                    logger.warning(f"[MessageState.add_message] db_manager attributes: {dir(self.db_manager)}")
                else:

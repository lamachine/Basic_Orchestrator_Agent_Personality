...
from src.services.mcp_services.mcp_adapter import PENDING_MCP_REQUESTS, MCPAdapter, check_mcp_status
...
def generate_tool_description(endpoint_name: str, capability: str) -> str:
    ...
    descriptions = {
        ...
        "postgres": {"query": "Run a read-only SQL query"},
    }
    ...
    try:
        ...
        for task_id, response in list(PENDING_MCP_REQUESTS.items()):
            ...
            if status in ["completed", "error"] and task_id in PENDING_TOOL_REQUESTS:
                ...
                if PENDING_TOOL_REQUESTS[task_id].get("status") in [
                    "completed",
                    "error",
                ]:

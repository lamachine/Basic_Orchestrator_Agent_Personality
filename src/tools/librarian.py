...
def _process_task(task: str, request_id: str):
    ...
    try:
        ...
        response = {
            "status": "success",  # Restore original status
        ...
        try:
            ...
            PENDING_TOOL_REQUESTS[request_id] = {
                "name": "librarian",
                "status": "completed",  # Use "completed" status in the tracking system
...
def librarian_tool(task: str, request_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    response = {
        ...
        "name": "librarian",
        "request_id": request_id  # Make sure request_id is at the top level consistently
    }

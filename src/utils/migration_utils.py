...
async def get_applied_migrations(db_manager: DatabaseManager) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        result = await db_manager.db_service.select_records(
            table_name=MIGRATION_HISTORY_TABLE, order_by="applied_at"
        )
...
async def run_migration(db_manager: DatabaseManager, migration_file: str) -> bool:
    ...
    migration_record = {"name": name, "filename": filename, "status": "running"}
    ...
    try:
        ...
        await db_manager.db_service.insert(
            table_name=MIGRATION_HISTORY_TABLE, data=migration_record
        )
...
async def run_pending_migrations(db_manager: DatabaseManager) -> Dict[str, Any]:
    ...
    results = {"total": len(pending_files), "success": 0, "failed": 0, "details": []}
    ...
    for filename in pending_files:
        ...
        results["details"].append({"filename": filename, "success": success})
...
def create_table_migration(table_name: str, columns: List[Dict[str, Any]]) -> str:
    ...
    filepath = create_migration_file(f"create_{table_name}_table", f"Create {table_name} table")
...
async def create_migration_from_diff(
    ...
    # Create migration file
    filepath = create_migration_file(name, f"Migration from {source_table} to {target_table}")
        ...
        for col_name, source_col in source_cols.items():
            if col_name in target_cols:
                ...
                if source_col["data_type"] != target_col["data_type"]:
                    ...
                    f.write(
                        f"ALTER TABLE {source_table} ALTER COLUMN {col_name} TYPE {target_col['data_type']};\n\n"
                    )
                ...
                if source_col.get("is_nullable") != target_col.get("is_nullable"):
                    if target_col.get("is_nullable", "YES").upper() == "NO":
                        ...
                        f.write(
                            f"ALTER TABLE {source_table} ALTER COLUMN {col_name} SET NOT NULL;\n\n"
                        )
                    else:
                        f.write(f"-- Remove NOT NULL constraint from {col_name}\n")
                        f.write(
                            f"ALTER TABLE {source_table} ALTER COLUMN {col_name} DROP NOT NULL;\n\n"
                        )

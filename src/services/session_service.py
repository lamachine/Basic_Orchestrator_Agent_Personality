...
class SessionService:
    ...
    async def list_sessions(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            response = await self.db_service.select_records('sessions', filters={'user_id': user_id})
    ...
    async def search_sessions(self, query: str, user_id: str = "developer") -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"Found {len(results)} sessions matching query '{query}' for user '{user_id}'")
            return results
        except Exception as e:

"""Valet agent graph implementation."""
...
class ValetGraph(LocalGraph):
    """Valet agent graph implementation."""
    
    def __init__(self, parent_graph_id: Optional[str] = None):
        """Initialize the valet graph.
        
        Args:
            parent_graph_id: Optional ID of parent graph (usually orchestrator)
        """
        # Simple test prompt for now
        system_prompt = """You are a valet assistant. You help manage schedules, tasks, and personal affairs.
        
For testing purposes:
- If the user says "tool valet", respond with "Valet tool activated"
- Otherwise, be conversational and helpful about scheduling and task management
...
Remember: This is just for testing the graph structure. The real prompt will be added later."""
    ...
    async def process_message(self, message: str, request_id: Optional[str] = None) -> Dict[str, Any]:
        """Process a message through the valet graph.

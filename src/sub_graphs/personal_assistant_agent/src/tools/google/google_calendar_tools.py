...
def create_event(
    ...
    try:
        ...
        event = (
            service.events()
            .insert(calendarId="primary", body=event)  # Use primary instead of email
            .execute()
        )
...
def delete_event(event_id: str) -> str:
    ...
    try:
        ...
        service.events().delete(
            calendarId="primary", eventId=event_id  # Use primary instead of email
        ).execute()
...
def update_event(
    ...
    try:
        ...
        event = (
            service.events()
            .get(calendarId="primary", eventId=event_id)  # Use primary instead of email
            .execute()
        )
...
def manage_calendar_acl(
    ...
    try:
        ...
        if action == "list":
            ...
            rule = service.acl().get(calendarId=calendar_id, ruleId=rule_id).execute()
            return json.dumps(rule, indent=2)

        elif action == "insert" and role and scope_type and scope_value:
            rule = {"role": role, "scope": {"type": scope_type, "value": scope_value}}
            ...
            rule["role"] = role
            result = (
                service.acl().update(calendarId=calendar_id, ruleId=rule_id, body=rule).execute()
            )
            return json.dumps(result, indent=2)
...
def manage_calendar_list(
    ...
    try:
        ...
        if action == "list":
            ...
            result = service.calendarList().update(calendarId=calendar_id, body=calendar).execute()
            return json.dumps(result, indent=2)
...
def move_event(calendar_id: str, event_id: str, destination_id: str) -> str:
    ...
    try:
        ...
        event = (
            service.events()
            .move(calendarId=calendar_id, eventId=event_id, destination=destination_id)
            .execute()
        )
        return json.dumps(event, indent=2)
    except Exception as e:
...
def import_event(calendar_id: str, ical_data: str) -> str:
    ...
    try:
        service = get_calendar_service()
        event = (
            service.events().import_(calendarId=calendar_id, body={"iCalUID": ical_data}).execute()
        )
        return json.dumps(event, indent=2)
    except Exception as e:
...
if __name__ == "__main__":
    ...
    try:
        ...
        quick_event = quick_add_event(
            calendar_id="primary", text="Meeting with Team tomorrow at 2pm for 1 hour"
        )
        print(quick_event)

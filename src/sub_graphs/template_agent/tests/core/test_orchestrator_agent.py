...
def orchestrator_agent(orchestrator_config):
    ...
    agent.graph_state = {"conversation_state": {"add_message": AsyncMock()}}
...
def test_orchestrator_agent_config_validation():
    ...
    with pytest.raises(ValidationError):
        BaseAgentConfig(name="test", enable_history="not_a_boolean")  # Should be a boolean

    # Test nested validation
    with pytest.raises(ValidationError):
        # api_url should be a valid URL format
        BaseAgentConfig(name="test", api_url="invalid-url-format")

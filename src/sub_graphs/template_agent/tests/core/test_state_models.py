...
from src.common.state.state_models import State, StateRole, StateStatus, StateType


def test_state_creation():
    """Test basic state creation."""
    state = State(content="Test state", role=StateRole.USER, state_type=StateType.TEXT)
...
def test_state_types():
    ...
    state = State(content="Test code", role=StateRole.ASSISTANT, state_type=StateType.CODE)
    assert state.state_type == StateType.CODE

    state = State(content="Test error", role=StateRole.SYSTEM, state_type=StateType.ERROR)
...
def test_state_roles():
    ...
    state = State(content="Test state", role=StateRole.ASSISTANT, state_type=StateType.TEXT)
    assert state.role == StateRole.ASSISTANT

    state = State(content="Test state", role=StateRole.SYSTEM, state_type=StateType.TEXT)
...
def test_state_validation():
    ...
    with pytest.raises(ValueError):
        State(content="", role=StateRole.USER, state_type=StateType.TEXT)

    with pytest.raises(ValueError):
        State(content="Test state", role="invalid_role", state_type=StateType.TEXT)

    with pytest.raises(ValueError):
        State(content="Test state", role=StateRole.USER, state_type="invalid_type")

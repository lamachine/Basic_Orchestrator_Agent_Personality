...
from ...src.common.config import DBServiceConfig
...
def config() -> DBServiceConfig:
    ...
    return DBServiceConfig(
        ...
        enabled=True,
        config={"supabase_url": "http://localhost:54321", "supabase_key": "test_key"},
    )
...
async def test_select(db_service: DBService):
    ...
    mock_response = mocker.Mock()
    mock_response.data = [{"id": 1, "name": "test"}]
    db_service.client.table.return_value.select.return_value.eq.return_value.order.return_value.limit.return_value.execute.return_value = (
        mock_response
    )
    ...
    db_service.client.table.return_value.select.return_value.eq.assert_called_once_with(
        "name", "test"
    )
...
async def test_update(db_service: DBService):
    ...
    db_service.client.table.return_value.eq.return_value.update.return_value.execute.return_value = (
        mock_response
    )

    # Update records
    result = await db_service.update("test_table", {"name": "updated"}, {"id": 1})
...
async def test_delete_records(db_service: DBService):
    ...
    db_service.client.table.return_value.eq.return_value.delete.return_value.execute.return_value = (
        mock_response
    )

    # Delete records
    result = await db_service.delete_records("test_table", {"id": 1})
    assert result == [{"id": 1}]
...
async def test_get_messages(db_service: DBService):
    ...
    db_service.client.table.return_value.select.return_value.eq.return_value.order.return_value.execute.return_value = (
        mock_response
    )
...
async def test_search_messages(db_service: DBService):
    ...
    messages = await db_service.search_messages(
        query="test", session_id="test_session", user_id="test_user"
    )
    assert len(messages) == 1
    assert isinstance(messages[0], Message)
    assert messages[0].content == "Test message"
...
async def test_semantic_message_search(db_service: DBService):
    ...
    messages = await db_service.semantic_message_search(
        query="test", embedding=[0.1, 0.2, 0.3], limit=5
    )
    assert len(messages) == 1
    assert isinstance(messages[0], Message)
    assert messages[0].content == "Test message"
    ...
    db_service.client.rpc.assert_called_once_with(
        "match_documents",
        {"query_embedding": [0.1, 0.2, 0.3], "match_count": 5, "filter": None},
    )
...
async def test_health_check(db_service: DBService):
    ...
    db_service.client.from_.return_value.select.return_value.limit.return_value.execute.return_value = (
        mock_response
    )

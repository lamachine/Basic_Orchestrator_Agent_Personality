...
from ...src.common.state.state_models import (
    TaskStatus, Message, MessageStatus, MessageType, MessageRole
)
...
def test_validate_message_status_transition(validator):
    ...
    # Test case: Normal operation - valid transitions should pass
    assert validator.validate_message_status_transition(MessageStatus.PENDING, MessageStatus.RUNNING) is True
    assert validator.validate_message_status_transition(MessageStatus.RUNNING, MessageStatus.SUCCESS) is True
    assert validator.validate_message_status_transition(MessageStatus.RUNNING, MessageStatus.ERROR) is True
    assert validator.validate_message_status_transition(MessageStatus.RUNNING, MessageStatus.PARTIAL) is True
    assert validator.validate_message_status_transition(MessageStatus.PARTIAL, MessageStatus.SUCCESS) is True
    assert validator.validate_message_status_transition(MessageStatus.ERROR, MessageStatus.PENDING) is True
    
    # Test case: Error condition - invalid transitions should fail
    assert validator.validate_message_status_transition(MessageStatus.PENDING, MessageStatus.SUCCESS) is False
    assert validator.validate_message_status_transition(MessageStatus.SUCCESS, MessageStatus.RUNNING) is False
    assert validator.validate_message_status_transition(MessageStatus.SUCCESS, MessageStatus.ERROR) is False
    assert validator.validate_message_status_transition(MessageStatus.COMPLETED, MessageStatus.PARTIAL) is False

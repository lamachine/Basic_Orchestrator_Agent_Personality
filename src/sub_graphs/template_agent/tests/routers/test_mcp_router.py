...
from ...src.common.routers.mcp_router import MCPRequest, get_mcp_service, router
...
def mock_mcp_service():
    ...
    mock_service.call_mcp = AsyncMock(
        return_value={
            "status": "pending",
    ...
    mock_service.check_status = AsyncMock(
        return_value={
            "status": "completed",
            "task_id": "test-task-id",
...
def test_mcp_request_model_validation_error():
    ...
    with pytest.raises(ValueError):
        MCPRequest(
            endpoint_name="test_endpoint", capability="test_capability"
        )  # Missing parameters
...
async def test_mcp_endpoint(mock_mcp_service):
    ...
    with patch("src.common.routers.mcp_router.get_mcp_service", return_value=mock_mcp_service):
        result = await router.routes[0].endpoint("test_capability", request, mock_mcp_service)
...
async def test_mcp_endpoint_error(mock_mcp_service):
    ...
    with pytest.raises(HTTPException) as excinfo:
        with patch(
            "src.common.routers.mcp_router.get_mcp_service",
            return_value=mock_mcp_service,
        ):
            await router.routes[0].endpoint("test_capability", request, mock_mcp_service)
...
async def test_check_status(mock_mcp_service):
    ...
    # Test endpoint
    with patch("src.common.routers.mcp_router.get_mcp_service", return_value=mock_mcp_service):
        result = await router.routes[1].endpoint("test-task-id", mock_mcp_service)
...
async def test_check_status_error(mock_mcp_service):
    ...
    with pytest.raises(HTTPException) as excinfo:
        with patch(
            ...
            await router.routes[1].endpoint("nonexistent-task-id", mock_mcp_service)

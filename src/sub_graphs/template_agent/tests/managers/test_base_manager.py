...
from ...src.common.managers.state_models import Message, MessageType, MessageStatus, GraphState
from ...src.common.managers.service_models import ServiceConfig, ServiceCapability
...
def service_config():
    ...
    config.enabled = True
    config.get_capability = MagicMock(return_value=ServiceCapability(
...
def test_create_error_message(base_manager):
    ...
    error_message = base_manager.create_error_message(
        content="Test error",
        error_data={"error_code": 404, "reason": "Not found"}
    )
    ...
    # Test with minimal parameters
    minimal_error = base_manager.create_error_message(
        content="Minimal error"
    )

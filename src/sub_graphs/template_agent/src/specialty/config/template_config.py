    ...
    "properties": {
        "template_agent": {
            ...
            "properties": {
                ...
                "specialized_tools": {"type": "array", "items": {"type": "string"}},
        ...
        "template_graph": {
            "type": "object",
            "properties": {
                "custom_node_types": {"type": "array", "items": {"type": "string"}},
                "custom_edge_types": {"type": "array", "items": {"type": "string"}},
        ...
        "template_personality": {
            "type": "object",
            "properties": {
                "custom_traits": {"type": "array", "items": {"type": "string"}},
                "custom_goals": {"type": "array", "items": {"type": "string"}},
...
class TemplateAgentConfig(AgentConfig):
    ...
    enable_tool_specialization: bool = Field(
        default=False, description="Enable/disable tool specialization"
    )
    specialized_tools: List[str] = Field(
        default_factory=list, description="List of specialized tools"
    )
    ...
    def validate_specialized_tools(self) -> "TemplateAgentConfig":
        ...
        if self.enable_tool_specialization and not self.specialized_tools:
            raise ValueError(
                "specialized_tools must be set when enable_tool_specialization is True"
            )
        return self
...
class TemplateLoggingConfig(LoggingConfig):
    ...
    log_rotation_size: str = Field(
        default="100MB", pattern="^\\d+[KMG]B$", description="Log rotation size"
    )
    ...
    log_level: str = Field(
        ...
        description="Override base log_level",
    )
...
class TemplatePersonalityConfig(PersonalityConfig):
    ...
    system_prompt: str = Field(
        default="You are a helpful template agent.",
        description="Override base system_prompt",
    )
...
class TemplateConfig(BaseConfig):
    ...
    template_llm: Dict[str, TemplateLLMConfig] = Field(
        default_factory=lambda: {
            "default": TemplateLLMConfig(
                api_url="https://api.example.com/v1", default_model="gpt-4"
            )
        }
    )
    ...
    template_personality: TemplatePersonalityConfig = Field(
        default_factory=TemplatePersonalityConfig
    )

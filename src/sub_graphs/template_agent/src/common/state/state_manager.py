...
class StateManager:
    ...
    async def get_agent_state(self, session_id: str, agent_id: str) -> Dict[str, Any]:
        """Get an agent's state from a session."""
        session = await self.get_session(session_id)
        return session.conversation_state.get("agent_states", {}).get(agent_id, {})

    async def get_session_context(self, session_id: str, window_size: int = 5) -> List[Message]:
        """Get the context window for a session."""
        session = await self.get_session(session_id)
        return session.get_context_window(window_size)

    async def get_task_history(self, session_id: str) -> List[str]:

...
class ToolDescription(BaseModel):
    ...
    graph_state_type: Optional[Type[GraphState]] = Field(default=None, description="Type of graph state required")
...
class ToolRegistry:
    ...
    def list_tools(self, tool_type: Optional[ToolType] = None) -> List[ToolDescription]:
        ...
        if tool_type:
            return [
                self._tools[name]
                for name in self._tool_types[tool_type]
            ]
        return list(self._tools.values())
    
    def get_tool_descriptions_for_llm(self, tool_type: Optional[ToolType] = None) -> List[Dict[str, Any]]:
        """
    ...
    async def _discover_subgraph_tools(self) -> None:
        ...
        try:
            ...
            if parent_graph_dir.exists():
                await self._discover_tools_from_directory(parent_graph_dir / "tools", ToolType.SUBGRAPH)
        ...
        for name, param in sig.parameters.items():
            if name != 'graph_state':  # Skip graph_state parameter
                parameters[name] = ToolParameter(
                    ...
                    description=param.default.description if hasattr(param.default, 'description') else "",

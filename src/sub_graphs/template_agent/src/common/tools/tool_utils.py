...
async def handle_tool_calls(
    response_text: str, graph_state=None, agent_name: str = "template_agent"
) -> Dict[str, Any]:
        ...
        try:
            ...
            if graph_state:
                try:
                    ...
                    logger.error(
                        f"[{agent_name}] Failed to log tool call: {str(e)}",
                        exc_info=True,
                    )
...
async def execute_tool(
    ...
    parent_request_id = task.get("metadata", {}).get("parent_request_id")
    ...
    # Build clean args with request_id and parent_request_id in metadata
    args = {
        "task": task_str.strip(),
        "request_id": request_id,
    ...
    try:
        ...
        logger.error(f"[{agent_name}] Error executing tool {tool_name}: {str(e)}")
        ...
        PENDING_TOOL_REQUESTS[request_id]["response"] = error_result
        ...
        if graph_state:
            try:
                ...
                logger.error(
                    f"[{agent_name}] Failed to log tool error: {str(log_error)}",
                    exc_info=True,
                )
...
def format_tool_results(
    processing_result: Dict[str, Any], agent_name: str = "template_agent"
) -> str:

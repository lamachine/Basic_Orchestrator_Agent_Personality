...
class CentralDecisionMaker:
        ...
        if self.rag_engine:
            ...
            try:
                message_obj = SwarmMessage(
                    content=message, user_id=user_id, metadata=metadata or {}
                )
    ...
    def _format_cot_prompt(self, context: DecisionContext, memories: List[Dict[str, Any]]) -> str:
        """
...
"""
        ...
        for idx, interaction in enumerate(
            context.previous_interactions[-3:]
        ):  # Last 3 interactions
...
"""
    ...
    def make_decision(self, context: DecisionContext) -> DecisionResult:
        ...
        memories = self._retrieve_relevant_memories(
            user_id=context.user_id, query=context.user_message
        )
    ...
    def evaluate_tool_response(
        ...
        updated_context = DecisionContext(
            ...
            metadata={
                **context.metadata,
                "tool_response": (
                    tool_response.to_dict() if hasattr(tool_response, "to_dict") else tool_response
                ),
            },
        )

...
from src.sub_graphs.template_agent.src.common.tools.orchestrator_tools import check_completed_tool_requests, cleanup_processed_request
...
class CLIToolHandler:
                    ...
                    if (
                        ...
                        if hasattr(self.agent, "handle_tool_completion"):
                            ...
                            if response and response.get("status") == "success":
                                ...
                                logger.warning("Agent's tool completion handler returned unsuccessful response: %s", response)
                        else:
    ...
    def check_pending_completions(self) -> None:
        ...
        try:
            ...
            if completions:
                logger.debug("Found %d pending tool completions from previous sessions", len(completions))
                for request_id, completion in completions.items():
                    try:
                        if not completion.get("processed_by_agent", False):
                            logger.debug("Processing pending completion for request %s", request_id)
                            self.display_handler.display_tool_result({
                                "request_id": request_id,
                                "name": completion.get("name", "unknown"),
                                "response": completion.get("response", {})
                            })
                            logger.debug("Displayed pending completion for request %s", request_id)
                    except Exception as e:
                        logger.error("Error processing pending completion %s: %s", request_id, str(e))
                        logger.error("Traceback: %s", traceback.format_exc())
        except Exception as e:

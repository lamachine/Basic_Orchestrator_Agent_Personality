...
class CLISessionHandler:
    ...
    async def search_sessions(self) -> None:
        ...
        try:
            ...
            self.display_handler.display_message(
                {"role": "system", "content": "Enter search query: "}
            )
            ...
            self.display_handler.display_message({"role": "system", "content": "\nSearch results:"})
            ...
            for i, (session_id, info) in enumerate(results.items(), 1):
                ...
                logger.debug(
                    "Displaying search result %d: %s (Last active: %s)",
                    i,
                    name,
                    updated,
                )
            ...
            try:
                ...
                self.display_handler.display_message(
                    {"role": "system", "content": "Invalid choice."}
                )
            ...
            logger.error("Traceback: %s", traceback.format_exc())
            self.display_handler.display_message(
                {"role": "system", "content": "Error searching sessions."}
            )
    ...
    async def continue_session(self, session_id: str) -> None:
        ...
        try:
            ...
            self.display_handler.display_message(
                {"role": "system", "content": f"Resumed session: {self.session_name}"}
            )
        except Exception as e:
            logger.error("Error continuing session: %s", str(e))
            logger.error("Traceback: %s", traceback.format_exc())
            self.display_handler.display_message(
                {"role": "system", "content": "Error resuming session."}
            )

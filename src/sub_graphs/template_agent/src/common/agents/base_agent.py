...
class BaseAgent:
    ...
    def __init__(self, config: BaseAgentConfig):
        ...
        if config.api_url and config.model:
            ...
            self.llm = LLMService.get_instance(
                api_url=config.api_url, model=config.model, max_tokens=config.max_tokens
            )
        else:
            self.llm = None
    ...
    async def create_message(
        ...
        **kwargs,
        ...
        if parent_message:
            ...
            message_data["metadata"].update({"parent_context": parent_message.metadata})
    ...
    async def update_message_status(
        ...
        message.metadata.update({"status_updated_at": datetime.now().isoformat(), **kwargs})
    ...
    async def chat(self, user_input: str) -> Dict[str, Any]:
        ...
        try:
            ...
            error_message = await self.create_message(
                content=f"Error: {str(e)}", type=MessageType.ERROR
            )

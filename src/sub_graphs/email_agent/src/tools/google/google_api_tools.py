...
def get_credentials():
    ...
    if not client_secret_file or not os.path.exists(client_secret_file):
        raise ValueError(
            f"Client secret file not found. Please check GOOGLE_CLIENT_SECRET_FILE in .env"
        )
...
def gmail_drafts_update(
    draft_id: str, to: str = None, subject: str = None, body: str = None
) -> str:
...
def tasks_insert(tasklist_id: str, title: str, notes: str = "", due: str = None) -> str:
    """Create a new task."""
    try:
        service = get_tasks_service()
...
def tasks_move(tasklist_id: str, task_id: str, parent: str = None, previous: str = None) -> str:
    """Move a task."""
    try:
        service = get_tasks_service()
        result = (
...
def calendar_events_quick_add(calendar_id: str, text: str) -> str:
...
def calendar_freebusy_query(time_min: str, time_max: str, calendar_ids: List[str] = None) -> str:
...
def calendar_events_instances(calendar_id: str, event_id: str, max_results: int = 10) -> str:
    ...
    try:
        ...
        results = (
            service.events()
            .instances(calendarId=calendar_id, eventId=event_id, maxResults=max_results)
            .execute()
        )
        return json.dumps(results.get("items", []), indent=2)
    except Exception as e:
...
def tasks_tasklists_patch(tasklist_id: str, title: str) -> str:
    ...
    try:
        service = get_tasks_service()
        result = service.tasklists().patch(tasklist=tasklist_id, body={"title": title}).execute()
        return json.dumps(result, indent=2)
    except Exception as e:
...
def tasks_update(
    tasklist_id: str, task_id: str, title: str, notes: str = "", due: str = None
) -> str:
